...
async function runSchema() {
  ...
  const resetClassificaKey = 'reset_classifica_pdt_jump_20260429_finale';
  const resetClassificaCheck = await pool.query(
    'SELECT value FROM site_settings WHERE key = $1',
    [resetClassificaKey]
  );

  if (resetClassificaCheck.rows.length === 0) {
    ...
    console.log('Classifica PDT Jump resettata correttamente.');
  }
