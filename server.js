...
const RIONE_CRITERIA = ['Residenza', 'Domicilio', 'Legame familiare', 'Altro criterio approvato'];
...
app.post('/iscrizioni', async (req, res) => {
  ...
  if (errors.length) {
    return res.status(400).render('home', {
      title: 'Palio della Torre Leverano', sports, settings, formData: req.body, errors, rioni: RIONI, rioneCriteria: RIONE_CRITERIA,
    });
  }
  ...
  await pool.query(
    ...
    [
      sport_id, email.trim(), rione.trim(), fee_confirmation === 'yes',
      player1_full_name.trim(), player1_tax_code.trim().toUpperCase(), player1_phone.trim(), player1_rione_criteria.trim(), player1_rione_address.trim(), player1_shirt === 'yes',
      terms_rione_check === 'yes', terms_organizer_confirmation === 'yes', terms_privacy === 'yes', terms_images === 'yes', terms_liability === 'yes',
...
app.post('/admin/settings/update', requireAuth, async (req, res) => {
  const payload = {
    ...
    registration_fee_text: req.body.registration_fee_text || '___',
  };
